...
export default class PostUserProfile extends React.Component {
  ...
  makeRequesttoFollowOrUnFollowPostUser = () => {
    ...
    fetch(url, {
      ...
      .then(responseData => {
        console.debug(
          'Post USER Profile Page follow or unfollow response:',
          responseData,
        );
        this.setState({loading: false});
        if (responseData.status === 200) {
      ...
      .catch(error => {
        console.debug(
          'Postuser Profile Page follow or unfollow response ERROR:',
          error,
        );
        this.setState({error: error, loading: false});
        this.refs.toast.show('Something went wrong. Please try again later');
        ...
        <ScrollView style={{backgroundColor: 'white'}}>
          ...
          {showFolloworUnfollowButton ? (
            <TouchableOpacity
              ...
              onPress={() => {
                this.makeRequesttoFollowOrUnFollowPostUser();
              }}>
              <LinearGradient
                start={{x: 0, y: 0}}
                end={{x: 1, y: 0}}
                ...
                <Text style={AppStyle.appButton_Text}>
                  {isUserAlreadyFollowing ? 'UN FOLLOW' : 'FOLLOW'}
                </Text>
              </LinearGradient>
            </TouchableOpacity>
          ) : null}
